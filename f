...
-- Function to check the weapon in hand and set FOV based on the gun limits
local function getWeaponFov()
    local currentTool = LocalPlayer.Character:FindFirstChildOfClass("Tool")
    if currentTool then
        local toolName = currentTool.Name:lower()

        -- Check if the tool is DB or Revolver
        if toolName == "db" then
            return shared.Forbidden.Silent.GunsLimits.Db  -- Use Db limit for DB gun
        elseif toolName == "revolver" then
            return shared.Forbidden.Silent.GunsLimits.Revolver  -- Use Revolver limit for Revolver gun
        end
    end
    return nil  -- No specific gun, return nil to fall back on normal FOV calculation
end
...
local function updateFov()
    ...
    -- Check if we need to apply custom FOV for specific guns (DB or Revolver)
    local customFov = getWeaponFov()

    local dynamicFovSize
    if customFov then
        -- If we have a custom FOV for the gun, use that
        dynamicFovSize = customFov
    else
        -- Otherwise, dynamically calculate the FOV based on X, Y, Z values
        dynamicFovSize = calculateFov(settings.X, settings.Y, settings.Z)
    end
...
local function isPartInFovAndVisible(part)
    ...
    -- Dynamically calculate the FOV based on X, Y, Z values, or use custom FOV for guns
    local customFov = getWeaponFov()
    local dynamicFovSize
    if customFov then
        dynamicFovSize = customFov
    else
        dynamicFovSize = calculateFov(shared.Forbidden.Silent.VoidLimits.X, shared.Forbidden.Silent.VoidLimits.Y, shared.Forbidden.Silent.VoidLimits.Z)
    end
